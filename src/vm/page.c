...
/* Helper function for common swap slot cleanup */
static void
spt_free_swap_slot (struct spt_entry *entry)
{
...
/* Helper function for common memory unloading operations */
static void
spt_unload_from_memory (struct spt_entry *entry)
{
  ASSERT (entry != NULL);
...
{
  /* Unload from memory using helper */
  spt_unload_from_memory (entry);
  
  /* Free swap slot if any (shouldn't happen during normal unload) */
  spt_free_swap_slot (entry);
}
