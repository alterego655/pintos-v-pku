...
swap_get_free_slots(void)
{
    ...
    lock_acquire(&swap_lock);
    size_t free_slots = bitmap_count(swap_bitmap, 0, swap_slot_count, false);
    lock_release(&swap_lock);
...
swap_is_full(void)
{
    return swap_get_free_slots() == 0;
}

/* Check if swap is available */
bool
swap_is_available(void)
{
    return swap_bitmap != NULL;
} 